...
public:
   ...
   char const * GetStorageClassName(uint8_t sc);   // Get storage class name
...
protected:
   ...
   uint32_t StringTableSize;                       // Size of string table (for object files)
   int NumberOfSymbols;                          // Number of symbol table entries (for object files)
   uint64_t ImageBase;                             // Image base (for executable files)
...
public:
   ...
   void PublicNames(CMemoryBuffer * Strings, CSList<SStringEntry> * Index, int m); // Make list of public names
protected:
   uint32_t NumRecords;                            // Number of records
   ...
   CSList<uint32_t> LocalNameOffset;               // Offset into NameBuffer of segment names by name index
   CSList<uint32_t> SegmentNameOffset;             // Offset into NameBuffer of segment names by segment index
   CSList<uint32_t> SymbolNameOffset;              // Offset into NameBuffer of external symbol names
   CSList<uint32_t> GroupNameOffset;               // Offset into NameBuffer of group names
   char * GetLocalName(uint32_t i);                // Get segment name by name index
   uint32_t GetLocalNameO(uint32_t i);               // Get segment name by converting name index offset into NameBuffer
   const char * GetSegmentName(uint32_t i);        // Get segment name by segment index
   const char * GetSymbolName(uint32_t i);         // Get external symbol name
   const char * GetGroupName(uint32_t i);          // Get group name by index
   static const char * GetRecordTypeName(uint32_t i);// Get OMF record type name
...
protected:
   ...
   uint32_t NumPublicSymbols;                       // Number of public symbols in new file
   uint32_t NumExternalSymbols;                     // Number of external symbols in new file
   uint32_t NumRelocations;                         // Number of entries in RelocationBuffer
};
...
protected:
   ...
   CSList<uint32_t> SegmentTranslation;            // Translate old segment number to new symbol table index
   CSList<uint32_t> ExtdefTranslation;             // Translate old external symbol number to new symbol table index
...
protected:
   ...
   void MakeBinaryFile();                        // Convert subfunction: Putting sections together
   int NumSectionsNew;                           // Number of sections in new file
   CArrayBuf<int32_t> NewSectIndex;                // Array of new section indices
   CArrayBuf<int32_t> SymbolsUsed;                 // Array of new symbol indices
   CSList<int32_t> NewSymbolIndex;                 // Buffer for array of new symbol indices
   ...
   uint32_t RawDataOffset;                         // File offset for raw data
   CFileBuffer ToFile;                           // File buffer for PE/COFF file
   SCOFF_FileHeader NewFileHeader;               // New file header
};
...
protected:
   ...
   void Elf2MacRelocations(Elf32_Shdr &, MAC_section_32 &, uint32_t NewRawDataOffset, uint32_t oldsec);
   void Elf2MacRelocations(Elf64_Shdr &, MAC_section_64 &, uint32_t NewRawDataOffset, uint32_t oldsec);
   ...
   uint32_t NumSymbols[4];               // Accumulated number of entries in each NewSymTab[]
   uint32_t NewSectHeadOffset;           // File offset to first section header
   uint32_t NewSymtabOffset;             // File offset to symtab command
   int NumSectionsNew;                 // Number of sections in new file
   uint32_t RawDataOffset;               // Offset to raw data in old file
   uint32_t NumOldSymbols;               // Number of symbols in old file
   uint32_t CommandOffset;               // Offset to first load command = segment header
};
...
protected:
   ...
   void MakeBinaryFile();                         // Convert subfunction: Putting sections together
   void TranslateAddress(MInt addr, uint32_t & section, uint32_t & offset); // Translate address to section + offset
   uint32_t MakeGOTEntry(int symbol);               // Make entry in fake GOT for symbol
   void MakeGOT();                                // Make fake Global Offset Table
   int symtab;                                    // Symbol table section number
   int shstrtab;                                  // Section name string table section number
   int strtab;                                    // Object name string table section number
   int stabstr;                                   // Debug string table section number
   uint32_t NumSectionsNew;                         // Number of sections generated for 'to' file
   uint32_t MaxSectionsNew;                         // Number of section buffers allocated for 'to' file
   uint32_t HasGOT;                                 // Contains references to global offset table
...
protected:
   ...
   void MakeBinaryFile();                        // Convert subfunction: Putting sections together
   uint32_t isymtab[2];                            // static and dynamic symbol table section number
   uint32_t istrtab[4];                            // string table section number: symbols, dynamic symbols, sections, debug
   ...
   CArrayBuf<uint32_t> NewSymbolIndex;             // Array for translating old to new symbol indices
   uint32_t NumOldSymbols;                         // Size of NewSymbolIndex table
   uint32_t FirstGlobalSymbol;                     // Index to first global symbol in .symtab
   CFileBuffer ToFile;                           // File buffer for modified PE file
};
...
protected:
   ...
   void ChangeSections(uint32_t HeaderOffset, uint32_t Num);// Convert subfunction: Change section names and relocation records if needed
   void ChangeImportTable(uint32_t FileOffset, uint32_t Num);// Convert subfunction: Change symbol indices in import table if needed
   void MakeBinaryFile();                        // Convert subfunction: Putting sections together
   int  NewSymbolIndex(int OldIndex);            // Convert subfunction: Translate old to new symbol index
   uint32_t NewFileOffset(uint32_t OldOffset);       // Convert subfunction: Translate old to new file offset
   MacSymbolTableBuilder<TMAC_nlist, MInt> NewSymbols[3];// Buffers for building new symbol tables: local, public, external
   CMemoryBuffer NewSymbolTable;                 // Buffer for building new symbol table
   CMemoryBuffer NewStringTable;                 // Buffer for building new string table
   CFileBuffer ToFile;                           // File buffer for modified PE file
   uint32_t NumOldSymbols;                         // Size of NewSymbolIndex table
   uint32_t NewIlocalsym;	                         // index to local symbols
   uint32_t NewNlocalsym;	                         // number of local symbols 
   uint32_t NewIextdefsym;	                      // index to public symbols
   uint32_t NewNextdefsym;	                      // number of public symbols 
   uint32_t NewIundefsym;	                         // index to external symbols
   uint32_t NewNundefsym;	                         // number of external symbols
   uint32_t NewSymtabOffset;                       // Offset to new symbol table
   uint32_t NewStringtabOffset;                    // Offset to new string table
   uint32_t NewStringtabEnd;                       // Offset to end of new string table
   uint32_t OldTablesEnd;                          // End of old symbol table and string table
   int32_t  SizeDifference;                        // Size of new file minus size of old file
};
...
protected:
   CDisassembler Disasm;                         // Disassembler
   CArrayBuf<int32_t>SectionNumberTranslate;       // Translate section numbers in source file to section numbers in asm file
   CArrayBuf<uint32_t>SymbolTableOffset;           // Addend to add to symbol number for each symbol table
   int64_t ImageBase;                              // Image base if executable file
   uint32_t ExeType;                               // File type: 0 = object, 1 = DLL/shared object, 2 = executable
   uint32_t NumSymbols;                            // Number of symbols defined
...
protected:
   ...
   void MakeRelocations(int32_t Segment, uint32_t RecNum, uint32_t SOffset, uint32_t RSize, uint8_t * SData);// Make relocation list in Disasm
   CDisassembler Disasm;                         // Disassembler
   CSList<SOMFSegment> Segments;                 // Name, size, etc. of all segments
   CSList<uint32_t> ExtdefTranslation;             // Translate old external symbol number to disasm symbol table index
   CSList<uint32_t> PubdefTranslation;             // Translate old public symbol number to disasm symbol table index
   CMemoryBuffer SegmentData;                    // Binary segment data
   int32_t NumSegments;                            // Number of segments
   int32_t FirstComDatSection;                     // First COMDAT section. All sections before this are SEGDEF segments
};
